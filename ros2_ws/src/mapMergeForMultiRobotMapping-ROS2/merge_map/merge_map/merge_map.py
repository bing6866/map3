...
def merge_maps(map1, map2, map3):
    ...
    # 1. 计算三个地图的最小边界和最大边界
    min_x = min(map1.info.origin.position.x, map2.info.origin.position.x, map3.info.origin.position.x)
    min_y = min(map1.info.origin.position.y, map2.info.origin.position.y, map3.info.origin.position.y)
    ...
    for y in range(map1.info.height):
        for x in range(map1.info.width):
            ...
            if 0 <= merged_i < len(merged_map.data): # 增加越界保护
                merged_map.data[merged_i] = map1.data[i]

    # 3. 处理 Map 2 (只填充未知区域 -1)
    for y in range(map2.info.height):
        for x in range(map2.info.width):
            ...
            merged_i = merged_x + merged_y * merged_map.info.width
            if 0 <= merged_i < len(merged_map.data):
                if merged_map.data[merged_i] == -1:
                    merged_map.data[merged_i] = map2.data[i]

    # 4. 处理 Map 3 (只填充未知区域 -1)
    for y in range(map3.info.height):
        for x in range(map3.info.width):
            i = x + y * map3.info.width
            merged_x = int(np.floor((map3.info.origin.position.x + x * map3.info.resolution - min_x) / merged_map.info.resolution))
            merged_y = int(np.floor((map3.info.origin.position.y + y * map3.info.resolution - min_y) / merged_map.info.resolution))
            merged_i = merged_x + merged_y * merged_map.info.width
            if 0 <= merged_i < len(merged_map.data):
                if merged_map.data[merged_i] == -1:
...
class MergeMapNode(Node):
    def __init__(self):
        ...
        # 修复：使用不同的变量名存储 subscription，否则会被覆盖
        ...
        self.sub3 = self.create_subscription(OccupancyGrid, '/map3', self.map3_callback, 10)
    ...
    def map1_callback(self, msg):
        ...
        if self.map2 is not None and self.map3 is not None:
            msg = merge_maps(self.map1, self.map2, self.map3)
